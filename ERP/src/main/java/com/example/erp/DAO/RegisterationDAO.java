...
//String sql="insert into world.student(firstName,lastName,sex,dob,email,section,country,firstAttempt,subjects) values('"+p.getFirstName()+"','"+p.getLastName()+"','"+p.getSex()+"','"+p.getDob()+"','"+p.getEmail()+"','"+p.isFirstAttempt()+"','"+p.getCountry()+"',true,'"+convertListToDelimitedString(p.getSubjects())+"')"; 
		...
		String sql = "insert into registeration(referenceid,fullname,phonenumber,mailid,course) values('"
				+ p.getReferenceid() + "','" + p.getFullname() + "','" + p.getPhonenumber() + "','" + p.getMailid()
				+ "','" + p.getCourse() + "')";
	...
	public Registeration getStudentById(int id) {
		...
		return template.query("select * from registeration where ID=" + id, new ResultSetExtractor<Registeration>() {
			...
			public Registeration extractData(ResultSet rs) throws SQLException, DataAccessException {
	...
	public Registeration getCandidatesByreferenceID(int ref) {
		...
		return template.query(
				"select id,referenceid,fullname,phonenumber,mailid,course from registeration where referenceid=" + ref,
				new ResultSetExtractor<Registeration>() {
	...
	public Registeration getStudentByRef(int ref) {
		return template.query("select phonenumber from registeration where referenceid=" + ref,
				new ResultSetExtractor<Registeration>() {
	...
	public void updateMe(Registeration r) {
		String sql = "update registeration set fullname='" + r.getFullname() + "',phonenumber='" + r.getPhonenumber()
				+ "',mailid='" + r.getMailid() + "',course='" + r.getCourse() + "' where id=" + r.getId() + "";
	...
	public List<Registeration> getRegisteration() {
		...
		return template.query("select fullname,mailid,phonenumber from registeration where id=19",
				new ResultSetExtractor<List<Registeration>>() {
	...
	public void saveAdmission(Registeration re) {
		...
		String sql = "update registeration set dob='" + re.getDob() + "',gender='" + re.getGender() + "',nationality='"
				+ re.getNationality() + "',caste='" + re.getCaste() + "',address='" + re.getAddress() + "',p_name='"
				+ re.getP_name() + "',p_phonenumber='" + re.getP_phonenumber() + "',p_mailid='" + re.getP_mailid()
				+ "',p_address='" + re.getP_address() + "',sslc_board='" + re.getSslc_board() + "',s_percentage='"
				+ re.getS_percentage() + "',s_yop='" + re.getS_yop() + "',puc_board='" + re.getPuc_board()
				+ "',p_percentage='" + re.getP_percentage() + "',p_yop='" + re.getP_yop() + "',graduate_board='"
				+ re.getGraduate_board() + "',g_percentage='" + re.getG_percentage() + "',g_yop='" + re.getG_yop()
				+ "',course_type='" + re.getCourse_type() + "',first_language='" + re.getFirst_language()
				+ "',quota_caste='" + re.getQuota_caste() + "',quota_sports='" + re.getQuota_sports()
				+ "',quota_speciallyenabled='" + re.getQuota_speciallyenabled() + "',entrance_exam='"
				+ re.getEntrance_exam() + "',r_no='" + re.getR_no() + "',r_marks='" + re.getR_marks()
				+ "',entrance_exam2='" + re.getEntrance_exam2() + "',r_no2='" + re.getR_no2() + "',r_marks2='"
				+ re.getR_marks2() + "' where referenceid=" + re.getReferenceid() + "";
		System.out.println(sql);
		template.update(sql);
	...
	public Registeration getOTP(int refer) {
		...
		return template.query("select otp from registeration where referenceid=" + refer,
				new ResultSetExtractor<Registeration>() {
	...
	public List<Countries> retriveAllCountries() {
		// TODO Auto-generated method stub
		return template.query("select * from countries", new ResultSetExtractor<List<Countries>>() {

			public List<Countries> extractData(ResultSet rs) throws SQLException, DataAccessException {

				List<Countries> list = new ArrayList<Countries>();
				while (rs.next()) {
					Countries e = new Countries();

					e.setName(rs.getString(3));

					list.add(e);
					System.out.println("All program type" + (e.getId() + "" + e.getName()));
				}
				return list;
			}
		});
